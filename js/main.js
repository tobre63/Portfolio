...
(function () {
  ...
  (function () {
    ...
    document.querySelectorAll('#bioAge, #captionAge, #contactAge').forEach(el => { if (el) el.textContent = age; });
    ...
    if (statProjEl) statProjEl.textContent = milestone > 0 ? milestone + '+' : TOTAL_PROJECTS + '';

    // Footer "Last updated" — always current month & year
    const footerUpdEl = document.getElementById('footerUpdated');
    if (footerUpdEl) {
      footerUpdEl.textContent = new Date().toLocaleDateString('en-GB', { month: 'long', year: 'numeric' });
    }
  })();
  ...
  if (hasHover && cursorEl && ringEl) {
    ...
    document.addEventListener('mouseover', e => {
      const isInteractive = e.target.closest('a, button, .proj-card, .proj-row, .c-link, .s-link, .tool-tag, .case-cta, .cs-acc-trigger');
  ...
  const GAMES = [
    ...
    {
      type: 'Print Advertising · Design', name: 'Advertising',
      ...
      tools: ['Photoshop', 'InDesign', 'Print Design'], url: '',
      caseId: 'ad'
    },
